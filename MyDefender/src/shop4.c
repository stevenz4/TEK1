    ...
    if (gnl.case_c == 1) {
        if (gnl.event.type == sfEvtMouseButtonPressed) {
            ...
            if ((gnl.mouse_pos.x < 1630) && gnl.mouse_pos.x > 1455
                ...
                gnl.imoney >= 500 && gnl.fire_c == 0) {
                gnl.imoney = gnl.imoney - 500;
                gnl = str_money(gnl, gnl.imoney);
                gnl.fire_t[3].texture = sfTexture_createFromFile
...
def_t c_1(def_t gnl)
{
    if ((gnl.mouse_pos.x < 1812) && (gnl.mouse_pos.x > 1637) &&
        (gnl.mouse_pos.y < 662) && (gnl.mouse_pos.y > 440) &&
        gnl.imoney >= 1150 && gnl.fire2_c == 1 && gnl.fire3_c == 0) {
        gnl.imoney = gnl.imoney - 1150;
        gnl = str_money(gnl, gnl.imoney);
        gnl.fire_rect[3].top = 660;
        gnl.fire3_c = 1;
        return gnl;
    }
    if ((gnl.mouse_pos.x < 1630) && (gnl.mouse_pos.x > 1455) &&
        (gnl.mouse_pos.y < 662) && (gnl.mouse_pos.y > 440) &&
        gnl.imoney >= 850 && gnl.fire_c == 1 && gnl.fire2_c == 0) {
        gnl.imoney = gnl.imoney - 850;
        gnl = str_money(gnl, gnl.imoney);
        gnl.fire_rect[3].top = 330;
        gnl.fire2_c = 1;
        return gnl;
    }
    return gnl;
